// src/pages/transportadoras/TransportadoraEdit.jsx
...
import { useTransportadora, useUpdateTransportadora } from '@/hooks/useTransportadoras';
...
export default function TransportadoraEdit() {
  const navigate = useNavigate();
  const { id } = useParams();
  const { toast } = useToast();

  const { data, isLoading, error } = useTransportadora(id);
  const updateTransportadora = useUpdateTransportadora();
  ...
  useEffect(() => {
    if (data?.data) {
      ...
      setFormData({
        ...
        integracao_ocorrencia: transportadora.integracao_ocorrencia || ''
      });
    }
  }, [data]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await updateTransportadora.mutateAsync({ id, data: dataToSend });
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center gap-4">
        <Button
          variant="ghost"
          size="icon"
          onClick={() => navigate(`/transportadoras/${id}`)}
        >
          <ArrowLeft className="h-4 w-4" />
        </Button>
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Editar Transportadora</h1>
          <p className="text-muted-foreground mt-1">
            Atualize os dados da transportadora {formData.nome}
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Atualize os dados cadastrais da transportadora
            </CardDescription>
          </CardHeader>
        ...
        <div className="flex justify-end gap-4 mt-6">
          ...
          <Button type="submit" disabled={updateTransportadora.isLoading}>
            {updateTransportadora.isLoading ? (
              <>
